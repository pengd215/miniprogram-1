...
│                 微信云开发（CloudBase）                        │
│  ┌───────────┐  ┌───────────┐  ┌──────────────────────────┐  │
│  │ 云函数×13  │  │ 云数据库×6 │  │ 权限校验（服务端 OpenID+角色）│  │
...
| 流水日志 | `pages/logs/logs` | 出入库流水查询，按条件筛选，支持批量导出 |
...
| `managePermission` | 员工档案全生命周期管理：列表查询、新增员工、角色分配、在职/离职状态切换、删除员工（仅管理员，含管理员保护） |
| `exportFlowData` | 流水批量导出：按时间范围与出入库类型查询，联表补齐车型/参考价格/操作人，按角色控制导出范围 |
...
| `employees` | 员工账号（用户名、密码哈希、姓名、角色、在职/离职状态、绑定的 OpenID） |
...
- **2026-08-23** 产品列表新增按库存状态分类筛选功能，修复产品列表车型字段过长部分会撑破卡片挤占库存和库存状态的位置，修复下拉加载列表不加载的问题
