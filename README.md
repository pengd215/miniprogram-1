...
> 一个基于微信云开发的减振器配件库存管理小程序，从配件建档到出入库、流水追溯、库存预警、库位管理、客户报价、操作回退，覆盖汽配门店库存全流程。
...
面向企业内部员工提供配件建档、出入库、流水追踪、库存预警、仓库库位管理、操作回退防误删等功能，同时为下游客户提供独立的报价查询入口。已上线运行，代码即实际生产可用版本。

- 前端：微信小程序原生框架（WXML / WXSS / JS）
- 后端：微信云开发（云函数 + 云数据库 + 文件存储），无需自建服务器
- 数据一致性：库存扣减与流水记录使用数据库事务，保证原子性
- 操作安全：全页面操作快照 + 逆向回滚机制，防止误操作导致数据丢失
...
- **库存出入库**：入库/出库操作采用数据库事务，确保"库存变动 + 流水记录 + 操作快照"同时成功或同时失败；出库自动校验库存是否充足
...
- **一键回退**：在流水日志页面点击"↩ 回退"按钮，输入回退原因即可恢复数据
...
| `product_update` 产品编辑 | 编辑配件页面保存 | 用快照覆盖当前数据 | OE码、库存、车型等12个字段恢复到编辑前 |
| `product_create` 产品创建 | 新建配件页面提交 | 软删除（标记 deleted） | 产品标记为已删除，关联流水作废 |

> ⚠️ **注意**：只有部署新版本云函数后产生的操作才会带有快照ID并支持回退，旧历史数据无法回退。
...
┌─────────────────────────────────────────────────────────────┐
│                    微信小程序前端（miniprogram/）               │
│登录 / 工作台 / 产品列表 / 出入库 / 流水(含回退) / 仓库 / 权限 / 报价│
└───────────────┬─────────────────────────────────────────────┘
...
| 流水日志 | `pages/logs/logs` |出入库流水查询、条件筛选、批量导出、🆕操作回退入口 |
| 个人中心 | `pages/employee/profile` | 个人资料展示 |
| 新建配件 | `pages/createPart/index` | 配件建档（支持图片上传，创建后可回退） |
| 编辑配件 | `pages/editPart/index` | 档案编辑、状态完善、单品预警值设置（保存后可回退） |
| 待办列表 | `pages/pending/index` | 待完善配件档案清单 |
| 入库 / 出库 | `pages/inbound/inbound`、`pages/outbound/outbound` | 数量录入、备注、提交（提交后可回退） |
...
| 员工权限 | `pages/permission/index` | 员工新增/角色分配/在职离职切换/删除（二次确认），关键词与角色分类筛选（仅管理员） |
| 客户报价 | `pages/client/search` | 客户报价查询入口，实时监听数据库变化自动刷新，支持图片预览 |
...
| `transaction_logs` | 出入库流水（类型、数量、操作人、时间、备注、快照关联ID） | v1.1  |
...
| `related_log_id` | string | 关联的 `transaction_logs` 记录ID |
...
│   ├── submitInbound/    # 入库云函数
│   ├── submitOutbound/   # 出库云函数
│   └── ...
├── docs/                 # 项目文档
│   └── 操作回退功能实现方案.md  #回退功能详细设计文档
...
- 操作快照机制：每次关键操作前保存完整数据快照，支持精确回退到操作前状态
- 回退权限控制：仅管理员和仓库主管可执行回退操作，普通员工无权回退
- 防连环回退：同一操作不可重复回退，回退操作本身也会记录审计日志
...
A：不可以。只有在部署新版本云函数（`submitInbound`、`submitOutbound`、`createSnapshot`）之后产生的操作才会带有快照ID，旧历史数据没有快照无法回退。
